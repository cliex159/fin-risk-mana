...
```
Since $skewness=`r round(skewness(n),2)`$ approximately equal $0$, the distribution is not skew and $mean=`r round(mean(n),2)`$ approximately equal $median=`r round(median(n),2)`$.
...
```
Since $skewness=`r round(skewness(t),2)`$ approximately equal $0$, the distribution is not skew and $mean=`r round(mean(t),2)`$ approximately equal $median=`r round(median(t),2)`$.
...
```
Since $skewness=`r round(skewness(sr),2)`$ is greater than $0$, the distribution is right-skew and $mean=`r round(mean(sr),2)`$ is greater than $median=`r round(median(sr),2)`$.
...
```
Since $skewness=`r round(skewness(sl),2)`$ is less than $0$, the distribution is left-skew and $mean=`r round(mean(sl),2)`$ is less than $median=`r round(median(sl),2)`$.
